...
import { canonicalDepartmentFunctionalGroup } from '../organization';
...
export function expectedMedicalTypeForDepartment(
  ...
  const functionalGroup = canonicalDepartmentFunctionalGroup(departmentName);
  if (
  ...
  if (functionalGroup === 'MAGAZYN') {
    return 'MAGAZYNIER';
  }
  if (functionalGroup === 'HEADLINER' || functionalGroup === 'PU') {
